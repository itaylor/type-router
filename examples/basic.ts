...
const userRouter = createRouter(
  [
    ...
    },
    {
      ...
      onEnter: (params) => {
        console.log(
          `Viewing post: ${params.slug} in category: ${params.category}`,
        );
      },
    },
  ] as const,
...
// @ts-expect-error non-existent isn't a valid route
await advancedRouter.navigate('/non-existent');
// This will trigger onMiss and use the fallback route
await advancedRouter.navigateAny('/non-existent');
...
const _hashRouter = createRouter(
  [
    { path: '/' },
    { path: '/app' },
    { path: '/app/:section' },
